...
const SeriesCredit = ({ credit }: SeriesCreditProps) => {
  const { _links, voice } = credit;

  const handlePress = () => {
    const seriesId = _links.show.href.split('/').pop();
    if (seriesId) {
    ...
    >
      <View className="flex-row items-start justify-between">
        <View className="flex-1">
          <Text
            className="text-lg font-sans-semibold text-aqua mb-1"
            numberOfLines={2}
          >
            {_links.show.name}
          </Text>
          <Text className="text-sm text-text-muted font-sans-regular">
            as {_links.character.name}
          </Text>
          {voice && (
